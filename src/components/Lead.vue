...
const obtainDataMethodForList = async () => { // 向后端获取列表数据列表
    try {
        ...
        let { data: { code, msg, dataCount: dc, list } } = await proxy.$axios.post("lead/list", 'strCurrentPageNo=' + currentPage.value + '&strPageSize=' + pageSize.value);
...
const addEntity = async () => { // 调用后端新增的方法
    try {
        ...
        let { data: { code, msg, idJustInserted } } = await proxy.$axios.post("lead/add", entityObject);
        if (code == 1 && msg == "SUCCESS") {
...
const modifyEntity = async () => { // 调用后端修改对象的方法
    try {
        ...
        let { data: { code, msg } } = await proxy.$axios.post("lead/modify", entityObject);
        if (code == 1 && msg == "SUCCESS") {
            obtainDataMethodForList();
        } else {
...
const removeEntity = async (id: string) => { // 调用后端删除实体的方法
    try {
        ...
        let { data: { code, msg } } = await proxy.$axios.post("lead/remove", 'id=' + id);
        if (code == 1 && msg == "SUCCESS") {
            obtainDataMethodForList();
        } else {
...
<template>
    <div class="lead">
        <el-table :data="arrTableData" style="width: 100%">
            ...
            <el-table-column prop="id" label="线索编号" width="90"> </el-table-column>
            <el-table-column prop="source" label="线索来源" width="180"></el-table-column>
        ...
        <el-drawer v-model="drawer" :title="elDrawerTitle" :direction="direction">
            <span>
                <el-form ref="ruleForm1" :model="entityObject" label-width="120px" size="small">
                    ...
                    <el-form-item label="线索来源" prop="source" :rules="[
