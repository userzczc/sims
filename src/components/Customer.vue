...
const obtainDataMethodForList = async () => { // 向后端获取列表数据列表
    try {
        ...
        let { data: { code, msg, dataCount: dc, list } } = await proxy.$axios.post("customer/list", 'strCurrentPageNo=' + currentPage.value + '&strPageSize=' + pageSize.value);
...
const addEntity = async () => { // 调用后端新增的方法
    try {
        ...
        let { data: { code, msg, idJustInserted } } = await proxy.$axios.post("customer/add", entityObject);
        if (code == 1 && msg == "SUCCESS") {
...
const handleEdit = (index: number, row: any) => { // 将抽屉的标题改为修改并将修改那行的数据赋给对象
    ...
    entityObject.evaluate = row.evaluate;
};

const modifyEntity = async () => { // 调用后端修改对象的方法
    try {
        // =========>第7处需要修改的地方：调用后端修改方法的路径<=========
        let { data: { code, msg } } = await proxy.$axios.post("customer/modify", entityObject);
        if (code == 1 && msg == "SUCCESS") {
            obtainDataMethodForList();
        } else {
...
const removeEntity = async (id: string) => { // 调用后端删除实体的方法
    try {
        ...
        let { data: { code, msg } } = await proxy.$axios.post("customer/remove", 'id=' + id);
        if (code == 1 && msg == "SUCCESS") {
            obtainDataMethodForList();
        } else {
...
<template>
    <div class="customer">
        <el-table :data="arrTableData" style="width: 100%">
            ...
            <el-table-column prop="id" label="客户编号" width="90"> </el-table-column>
            ...
            <el-table-column prop="description" label="互动记录描述" width="180"></el-table-column>
            <el-table-column prop="evaluate" label="客户价值评估" width="180"></el-table-column>
            ...
            <span>
                <el-form ref="ruleForm1" :model="entityObject" label-width="120px" size="small">
                    ...
                    <el-form-item label="联系人" prop="name" :rules="[
                        ...
                        <el-input v-model="entityObject.name" placeholder="联系人"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="互动记录描述" prop="description" :rules="[
                        { required: true, message: 'Value is required', trigger: 'blur' },
                        { min: 1, max: 45, message: 'Length should be 1 to 45', trigger: 'blur' },
                    ]">
                        <el-input v-model="entityObject.description" placeholder="互动记录描述"></el-input>
                    </el-form-item>

                    <el-form-item label="客户价值评估" prop="evaluate" :rules="[
                        { required: true, message: 'Value is required', trigger: 'blur' },
                        { min: 1, max: 45, message: 'Length should be 1 to 45', trigger: 'blur' },
                    ]">
                        <el-input v-model="entityObject.evaluate" placeholder="客户价值评估"></el-input>
                    </el-form-item>
