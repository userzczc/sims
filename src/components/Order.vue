...
const obtainDataMethodForList = async () => { // 向后端获取列表数据列表
    try {
        ...
        let { data: { code, msg, dataCount: dc, list } } = await proxy.$axios.post("order/list", 'strCurrentPageNo=' + currentPage.value + '&strPageSize=' + pageSize.value);
...
const addEntity = async () => { // 调用后端新增的方法
    try {
        ...
        let { data: { code, msg, idJustInserted } } = await proxy.$axios.post("order/add", entityObject);
        if (code == 1 && msg == "SUCCESS") {
...
const handleEdit = (index: number, row: any) => { // 将抽屉的标题改为修改并将修改那行的数据赋给对象
    ...
    entityObject.detail = row.detail;
    entityObject.contact = row.contact;
    entityObject.status = row.status;
};

const modifyEntity = async () => { // 调用后端修改对象的方法
    try {
        // =========>第7处需要修改的地方：调用后端修改方法的路径<=========
        let { data: { code, msg } } = await proxy.$axios.post("order/modify", entityObject);
        if (code == 1 && msg == "SUCCESS") {
            obtainDataMethodForList();
        } else {
...
const removeEntity = async (id: string) => { // 调用后端删除实体的方法
    try {
        ...
        let { data: { code, msg } } = await proxy.$axios.post("order/remove", 'id=' + id);
        if (code == 1 && msg == "SUCCESS") {
            obtainDataMethodForList();
        } else {
...
<template>
    <div class="order">
        <el-table :data="arrTableData" style="width: 100%">
            ...
            <el-table-column prop="detail" label="产品明细" width="180"></el-table-column>
            <el-table-column prop="contact" label="发货信息" width="270"></el-table-column>
            <el-table-column prop="status" label="付款状态" width="180"></el-table-column>
        ...
        <el-drawer v-model="drawer" :title="elDrawerTitle" :direction="direction">
            <span>
                <el-form ref="ruleForm1" :model="entityObject" label-width="120px" size="small">
                    ...
                    <el-form-item label="产品明细" prop="detail" :rules="[
                        { required: true, message: 'Value is required', trigger: 'blur' },
                        { min: 1, max: 45, message: 'Length should be 1 to 45', trigger: 'blur' },
                    ]">
                        <el-input v-model="entityObject.detail" placeholder="产品明细"></el-input>
                    </el-form-item>

                    <el-form-item label="发货信息" prop="contact" :rules="[
                        { required: true, message: 'Value is required', trigger: 'blur' },
                        { min: 1, max: 45, message: 'Length should be 1 to 45', trigger: 'blur' },
                    ]">
                        <el-input v-model="entityObject.contact" placeholder="发货信息"></el-input>
                    </el-form-item>

                    <el-form-item label="付款状态" prop="status" :rules="[
                        { required: true, message: 'Value is required', trigger: 'blur' },
                        { min: 1, max: 45, message: 'Length should be 1 to 45', trigger: 'blur' },
                    ]">
                        <el-input v-model="entityObject.status" placeholder="付款状态"></el-input>
                    </el-form-item>
